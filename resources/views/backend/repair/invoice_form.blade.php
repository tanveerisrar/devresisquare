        ...
        <div class="row">
            ...
            <div class="col-6">
                ...
                <input type="hidden" id="existingInvoiceToId"
                    value="{{ old('invoice_to_id', $repairIssue->workOrder->invoice_to_id ?? '') }}">
    ...
    <div class="row">
        <div class="col mb-3">
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="charge_to_landlord" class="form-control" value="{{ old('charge_to_landlord', $repairIssue->workOrder->charge_to_landlord ?? '' ) }}">
            </div>
        </div>
    </div>
    ...
    <table class="table">
        ...
        <tbody id="invoice-items">
            ...
            <tr>
                ...
                <td><input type="number" name="items[{{ $index }}][unit_price]" class="form-control unit-price_invoice" value="{{ $item->unit_price }}" required></td>
                <td><input type="number" name="items[{{ $index }}][quantity]" class="form-control quantity_invoice" value="{{ $item->quantity }}" required></td>
                {{-- <td>
                    <select name="items[{{ $index }}][tax_name]" class="form-control tax-name_invoice">
                        @foreach($taxRates as $taxRate)
                            <option value="{{ $taxRate->id }}" data-rate="{{ $taxRate->rate }}"
                                {{ (!empty($item->tax_rate_id) && $item->tax_rate_id == $taxRate->id) ? 'selected' : 
                ...
                <td><input type="number" name="items[{{ $index }}][tax_rate]" class="form-control tax-rate_invoice" value="{{ $item->tax_rate ?? '' }}" required></td>             
