...
<div class="d-flex justify-content-end gap-3 mb-3">
    ...
    <a class="btn btn-primary" href="{{ route('admin.repair.workorder.invoice', $repairIssue->id) }}">{{ $repairIssue->workOrder ? 'Edit Work Order & Invoice' : 'Create Work Order & Invoice' }}</a>
        ...
        @foreach($formSections as $section)
            ...
            <div class="accordion-item">
                ...
                <div id="collapse-{{ $formType }}" class="accordion-collapse collapse show" aria-labelledby="heading-{{ $formType }}">
                    <button class="btn btn-primary float-end editForm" data-form="{{ $formType }}" data-id="{{ $repairIssue->id }}">
                        Edit
                    </button>
