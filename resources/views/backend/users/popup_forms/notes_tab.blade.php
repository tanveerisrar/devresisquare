@if(!isset($editMode) || !$editMode)
    ...
    <!-- Add New Note Card -->
    <div class="mb-3">
        <button class="btn btn-outline-primary addForm" data-form="notes_tab" data-id="{{ $user->id }}">
            Add New Note
        </button>
    </div>
    <div class="row g-3 mb-3 note-list">
        <form method="GET" class="row g-3 mb-4">
            ...
            <div class="col-md-3">
                ...
                <a href="{{ route('admin.users.show', $user->id) }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
                ...
                <div class="card shadow-sm h-100">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        {{-- <button class="btn btn-outline-danger btn-sm editForm" data-form="notes_tab"
                            data-id="{{ $user->id }}" data-note-id="{{ $note->id }}" title="Edit Note">
...
@else
    <form id="userNotesTabForm">
        @csrf
        <input type="hidden" name="user_id" value="{{ $user->id }}">
        <input type="hidden" name="form_type" value="notes_tab">
        <input type="hidden" name="note_id" value="{{ $note->id ?? '' }}">
        {{-- new, generic noteable fields --}}
        <input type="hidden" name="noteable_type" value="{{ get_class($user) }}">
        <input type="hidden" name="noteable_id"   value="{{ $user->id }}">
